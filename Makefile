...
##### PROJECT VARIABLES
PROJECT_NAME				= OcamlCordovaDialogs
VERSION						= 1.0.0
...
ML_FILES 					=	$(ML_DIRECTORY)/dialogs.ml \
								$(ML_DIRECTORY)/test.ml
MLI_FILES 					=	$(ML_DIRECTORY)/dialogs.mli
...
gen_js_api:
	...
	ocamlfind ocamlc -o $(TMP_OUT_BYTECODE) -no-check-prims -I $(ML_DIRECTORY) -package gen_js_api -package js_of_ocaml -linkpkg $(ML_FILES)
	$(CC_JS) +gen_js_api/ojs_runtime.js $(TMP_OUT_BYTECODE)
